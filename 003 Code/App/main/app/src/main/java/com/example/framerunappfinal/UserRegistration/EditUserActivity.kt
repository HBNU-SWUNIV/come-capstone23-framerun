...
import com.example.framerunappfinal.databinding.ActivityEditUserBinding
import com.example.framerunappfinal.postUsername
...
class EditUserActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_AUDIO_RECORDING && resultCode == RESULT_OK) {
            recordedAudioPath = data?.getStringExtra("recordedAudioPath")

            Log.d(TAG,"recordedAudioPath : $recordedAudioPath")

            // 파일 이름을 추출하고 텍스트 뷰에 설정
...
//    private fun uploadImages(user: User, imageList: ArrayList<Uri>, fileUrls: ArrayList<String>) {
//
//
//        val storage = FirebaseStorage.getInstance()
//        val storageRef = storage.reference
...
//    private fun uploadImages(user: User, imageList: ArrayList<Uri>, fileUrls: ArrayList<String>,storageRef: StorageReference) {
//        if (imageList.isNotEmpty()) {
//            // 이미지 업로드를 위한 함수 호출
//            val imageUri = imageList.removeAt(0)
//            val imageName = "${user.title}/image_${System.currentTimeMillis()}.jpg"
//            val imageRef = storageRef.child(imageName)
//
//            imageRef.putFile(imageUri).continueWithTask { task ->
//                if(!task.isSuccessful){
...
//            }.addOnSuccessListener { uri ->
//                fileUrls.add(uri.toString())
//                uploadImages(user, imageList,fileUrls,storageRef)
//            }.addOnFailureListener { exception ->
//                Log.e(TAG,"Image upload failed", exception)
//            }
//        }else{
...
//    private fun uploadDataToFirebase(user: User, imageList: ArrayList<Uri>, audioUriString: String?) {
//        // Firebase Storage 초기화
//        val storage = FirebaseStorage.getInstance()
//        val storageRef = storage.reference
//
...
//    //스토리지에 사진 올리기
//    private fun uploadDataToFirebase(user: User, imageList: ArrayList<Uri>, audioUri :String?) {
//
//        //스토리지 초기화
...
//    }
    ...
    companion object {
        ...
        private const val REQUEST_AUDIO_RECORDING = 1
    }
